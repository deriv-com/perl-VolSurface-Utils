...
sub _calculate_strike_for_vanilla_put {
    ...
    for (my $i = 1; $i <= 5 * $premium_adjusted; $i++) {
        ...
        my $option_price_1 = Math::Business::BlackScholes::NonBinaries::vanilla_put($S, $k1, $time_in_years, $r, $r - $d, $sigma);
        ...
        my $option_price_2 = Math::Business::BlackScholes::NonBinaries::vanilla_put($S, $k1 * 1.000001, $time_in_years, $r, $r - $d, $sigma);
...
sub _calculate_strike_for_vanilla_call {
    ...
    for (my $i = 1; $i <= 5 * $premium_adjusted; $i++) {
        ...
        # Step 2: Calculate option price and the corresponding delta
        my $option_price_1 = Math::Business::BlackScholes::NonBinaries::vanilla_call($S, $k1, $time_in_years, $r, $r - $d, $sigma);
        my $delta_1 = Math::Business::BlackScholes::Binaries::Greeks::Delta::vanilla_call($S, $k1, $time_in_years, $r, $r - $d, $sigma);

        # Step 3: Numerically evaluate option at slightly different strike and calculate its corresponding delta
        my $option_price_2 = Math::Business::BlackScholes::NonBinaries::vanilla_call($S, $k1 * 1.000001, $time_in_years, $r, $r - $d, $sigma);
...
sub _strangle_difference {
    ...
    my $call_with_consistent_vol =
        Math::Business::BlackScholes::NonBinaries::vanilla_call($S, $market_conventional_call_strike, $tiy, $r, $r - $d,
        $market_conventional_call_sigma);

    my $put_with_consistent_vol =
        Math::Business::BlackScholes::NonBinaries::vanilla_put($S, $market_conventional_put_strike, $tiy, $r, $r - $d, $market_conventional_put_sigma);
    ...
    my $call_with_market_conventional_bf =
        Math::Business::BlackScholes::NonBinaries::vanilla_call($S, $market_conventional_call_strike, $tiy, $r, $r - $d, $market_conventional_bf);
    my $put_with_market_conventional_bf =
        Math::Business::BlackScholes::NonBinaries::vanilla_put($S, $market_conventional_put_strike, $tiy, $r, $r - $d, $market_conventional_bf);
...
sub _implied_vol {
    ...
    while (abs($esc) > 0.000001) {
        ...
        if ($type eq 'VANILLA_CALL') {
            $option_price = Math::Business::BlackScholes::NonBinaries::vanilla_call($S, $k, $tiy, $r, $r - $d, $vol);
            $vega = Math::Business::BlackScholes::Binaries::Greeks::Vega::vanilla_call($S, $k, $tiy, $r, $r - $d, $vol);
        } else {
            $option_price = Math::Business::BlackScholes::NonBinaries::vanilla_put($S, $k, $tiy, $r, $r - $d, $vol);
